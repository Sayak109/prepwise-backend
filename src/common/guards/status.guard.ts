...
export class AccountStatusGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const requiredStatus = this.reflector.getAllAndOverride<Account[]>(
      ACCOUNT_KEY,
...
export class ApprovalStatusGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const requiredStatus = this.reflector.getAllAndOverride<Approve[]>(
      APPROVAL_KEY,
      [context.getHandler(), context.getClass()],
    );
...
export class OnboardingStatusGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const requiredStatus = this.reflector.getAllAndOverride<Onboarding[]>(
      ONBOARDING_KEY,
      [context.getHandler(), context.getClass()],
    );
