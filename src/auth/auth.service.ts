...
export class AuthService {
  ...
  async checkUser(commonDto: CommonDto) {
    ...
    const credential =
      payload?.email ?? payload?.phone_no ?? payload?.['credential'];
  ...
  async registerStudent(commonDto: CommonDto, req: Request): Promise<Tokens> {
    const payload = this.getBody<RegisterDto>(commonDto);
    await this.register({
      ...payload,
    ...
    const user = await this.prisma.user.findUnique({
      where: { email: this.normalizeEmail(payload.email) },
      select: this.authUserSelect(),
    });
  ...
  private async verifyAuth(payload: AuthPayload, ip: string) {
    ...
    const email = this.normalizeEmail(payload.email);
    const phoneNo = payload.phone_no?.trim();
    const identifier = authMethod === AuthMethod.PHONE_OTP ? phoneNo : email;
  ...
  async updateUserProfile(userId: string, dto: userProfileDto) {
    ...
    const updated = await this.prisma.user.update({
      ...
      data: {
        ...
        passwordHash: dto.new_password
          ? await hashPassword(dto.new_password)
          : undefined,
      },
      select: this.publicUserSelect(),
    });
  ...
  async updateUser(_currentUser: any, userId: string, dto: UpdateUserDto) {
    ...
    const updated = await this.prisma.user.update({
      ...
      data: {
        ...
        email: dto.email ? this.normalizeEmail(dto.email) : undefined,
        role: dto.role
          ? this.toUserRole(dto.role, UserRole.STUDENT)
          : undefined,
        status: dto.status
          ? this.toStatus(dto.status, Status.ACTIVE)
          : undefined,
  ...
  private async handleProviderAuth(
    user: any,
    payload: AuthPayload,
    ip: string,
  ) {
  ...
  private extractRefreshToken(
    dto: refreshDto | CommonDto | string,
  ): string | null {
    const tokenValue =
